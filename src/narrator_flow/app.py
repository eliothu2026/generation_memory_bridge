...
# 处理一个 chunk（两种模式共用）
# ----------------------------------------------------------------------
def process_chunk(session: NarratorSession, chunk: TranscriptChunk) -> None:
...
if mode == "预制 Demo 播放":
    ...
    if "demo_session" not in st.session_state or st.session_state.get("demo_path") != selected_path:
    ...
    if (step_clicked or auto_play) and cursor < total:
        ...
        process_chunk(st.session_state.demo_session, chunk)
